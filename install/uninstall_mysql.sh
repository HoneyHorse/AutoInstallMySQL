...
process_h=`ps -ef|grep mysql|grep -v grep|grep -v install_mysql.sh|grep -v uninstall_mysql.sh|wc -l`
if [ $process_h -ne 0 ];then
    echo "`date +%Y-%m-%dC%T|sed 's/C/ /g'` Some MySQL processes is exists...plz check the service environment..."
    echo "`date +%Y-%m-%dC%T|sed 's/C/ /g'` Stop MySQL server..."
    pid=`ps -ef|grep mysql|grep -v grep|grep -v uninstall_mysql.sh|grep -v install_mysql.sh|awk '{print $2}'`
    for id in ${pid}
    do
        kill -9 ${id}
...
if [ -f "${cur_dir}/storagedir.txt" ]; then
    storagedir=`cat ${cur_dir}/storagedir.txt`
    if [ -d ${storagedir} ]; then
        rm -rf ${storagedir}
    fi
    defaultdir="\/opt\/mysql"
    newdir=${storagedir//\//\\/}
    sed -i "s/$newdir/$defaultdir/" ${cur_dir}/my.cnf
    rm -rf ${cur_dir}/storagedir.txt
else
    if [ -d "/opt/mysql" ]; then
        rm -rf /opt/mysql/*
    fi
...
echo "`date +%Y-%m-%dC%T|sed 's/C/ /g'` Now MySQL server is uninstall success..Thanks!!!!!!!.."
